using ASM_APDP.Models;

namespace ASM_APDP.Controllers.StudentManage
{
    public class StudentManagement
    {
        // Get all students from CSV
        public static List<Student> GetAllStudents()
        {
            var students = new List<Student>();
            var filePath = "wwwroot/Data/Students.csv"; // Path to the CSV file storing student data

            // Ensure the file exists
            if (File.Exists(filePath))
            {
                var lines = File.ReadAllLines(filePath); // Read all lines from the CSV file

                foreach (var line in lines.Skip(1))  // Skip the header line
                {
                    var values = line.Split(',');
                    students.Add(new Student
                    {
                        FullName = values[0],
                        Email = values[1],
                        Password = values[2],
                        DateOfBirth = values[3],
                        PhoneNumber = values[4],
                        Hometown = values[5],
                        Major = values[6]
                    });
                }
            }

            return students;
        }

        // Save the list of students to the CSV file
        public static void SaveStudents(List<Student> students)
        {
            var filePath = "wwwroot/Data/Students.csv";
            var lines = new List<string> { "FullName,Email,Password,DateOfBirth,PhoneNumber,Hometown,Major" }; // CSV header

            // Add each student to the list of lines
            foreach (var student in students)
            {
                lines.Add($"{student.FullName},{student.Email},{student.Password},{student.DateOfBirth},{student.PhoneNumber},{student.Hometown},{student.Major}");
            }

            // Write all lines to the CSV file
            File.WriteAllLines(filePath, lines);
        }

        // Check if the email already exists in the student list
        public static bool IsEmailExist(string email)
        {
            var students = GetAllStudents();  // Retrieve all students
            return students.Any(s => s.Email == email);  // Check if email exists
        }

        // Get student by email and old password (used for password change)
        public static Student GetStudentByEmail(string email, string oldPassword)
        {
            var filePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Data", "Students.csv");

            if (!File.Exists(filePath))
            {
                return null; // If file doesn't exist, return null
            }

            using (var reader = new StreamReader(filePath))
            {
                string line;
                bool isFirstLine = true;

                while ((line = reader.ReadLine()) != null)
                {
                    if (isFirstLine)
                    {
                        isFirstLine = false;
                        continue; // Skip header line
                    }

                    var data = line.Split(',');

                    // If email and old password match, return the student
                    if (data.Length == 7 && data[1] == email && data[2] == oldPassword)
                    {
                        return new Student
                        {
                            FullName = data[0],
                            Email = data[1],
                            Password = data[2],
                            DateOfBirth = data[3],
                            PhoneNumber = data[4],
                            Hometown = data[5],
                            Major = data[6]
                        };
                    }
                }
            }

            return null; // If no match is found
        }

        // Get student profile by email
        public static Student GetStudentProfile(string email)
        {
            var filePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Data", "Students.csv");

            if (!File.Exists(filePath))
            {
                return null; // If file doesn't exist, return null
            }

            using (var reader = new StreamReader(filePath))
            {
                string line;
                bool isFirstLine = true;

                while ((line = reader.ReadLine()) != null)
                {
                    if (isFirstLine)
                    {
                        isFirstLine = false;
                        continue; // Skip header line
                    }

                    var data = line.Split(',');

                    // If email matches, return the student profile
                    if (data.Length == 7 && data[1] == email)
                    {
                        return new Student
                        {
                            FullName = data[0],
                            Email = data[1],
                            Password = data[2],
                            DateOfBirth = data[3],
                            PhoneNumber = data[4],
                            Hometown = data[5],
                            Major = data[6]
                        };
                    }
                }
            }

            return null; // If no match is found
        }

        // Update student profile information
        public static bool UpdateStudentInfo(string email, string fullName, string dob, string phoneNumber, string hometown, string major)
        {
            var filePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Data", "Students.csv");
            var tempFilePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Data", "Students_temp.csv");

            if (!File.Exists(filePath))
            {
                return false; // If file doesn't exist, return false
            }

            bool isUpdated = false;

            using (var reader = new StreamReader(filePath))
            using (var writer = new StreamWriter(tempFilePath))
            {
                string line;
                bool isFirstLine = true;

                while ((line = reader.ReadLine()) != null)
                {
                    if (isFirstLine)
                    {
                        writer.WriteLine(line); // Write the header line
                        isFirstLine = false;
                        continue;
                    }

                    var data = line.Split(',');

                    if (data.Length == 7 && data[1] == email) // If email matches, update the student data
                    {
                        data[0] = fullName;
                        data[3] = dob;
                        data[4] = phoneNumber;
                        data[5] = hometown;
                        data[6] = major;
                        isUpdated = true;
                    }

                    writer.WriteLine(string.Join(",", data)); // Write updated or unchanged data
                }
            }

            // If updated, replace original file with the updated file
            if (isUpdated)
            {
                File.Delete(filePath);
                File.Move(tempFilePath, filePath);
            }

            return isUpdated;
        }

        // Update student password
        public static bool UpdatePassword(string email, string newPassword)
        {
            var filePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Data", "Students.csv");
            var tempFilePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Data", "Students_temp.csv");

            if (!File.Exists(filePath))
            {
                return false; // If file doesn't exist, return false
            }

            bool isUpdated = false;

            using (var reader = new StreamReader(filePath))
            using (var writer = new StreamWriter(tempFilePath))
            {
                string line;
                bool isFirstLine = true;

                while ((line = reader.ReadLine()) != null)
                {
                    if (isFirstLine)
                    {
                        writer.WriteLine(line); // Write header line
                        isFirstLine = false;
                        continue;
                    }

                    var data = line.Split(',');

                    if (data.Length == 7 && data[1] == email) // If email matches, update the password
                    {
                        data[2] = newPassword;
                        isUpdated = true;
                    }

                    writer.WriteLine(string.Join(",", data)); // Write updated or unchanged data
                }
            }

            // If updated, replace the original file
            if (isUpdated)
            {
                File.Delete(filePath);
                File.Move(tempFilePath, filePath);
            }

            return isUpdated;
        }
    }
}
